  ...
  for(i in 1:n_iter){
    if(include_std){
      power_std <- power_std + sapply(1:n_comp_obrien, function(t){
        rej <- stop_trial(intvs = rand_out[[i]]$contr_standard[t, k, 2:3], 
                          K = K, zero = min_thresh)
        rej
      })
    }
    power_rand <- power_rand + sapply(1:n_comp, function(t){
      rej <- stop_trial(intvs = rand_out[[i]]$contr[t, k, 2:3], 
                        K = K, zero = min_thresh)
      rej
    })
    power_ts <- power_ts + sapply(1:n_comp, function(t){
      rej <- stop_trial(intvs = ts_out[[i]]$contr[t, k, 2:3], 
                        K = K, zero = min_thresh)
      rej
    })
    power_rits <- power_rits + sapply(1:n_comp, function(t){
      rej <- stop_trial(intvs = rits_out[[i]]$contr[t, k, 2:3], 
                        K = K, zero = min_thresh)
      rej
    })
    if(include_ipw){
      power_rand_ipw <- power_rand_ipw + sapply(1:n_comp, function(t){
        rej <- stop_trial(intvs = rand_out[[i]]$contr_ipw[t, k, 2:3], 
                          K = K, zero = min_thresh)
        rej
      })
      power_ts_ipw <- power_ts_ipw + sapply(1:n_comp, function(t){
        rej <- stop_trial(intvs = ts_out[[i]]$contr_ipw[t, k, 2:3], 
                          K = K, zero = min_thresh)
        rej
      })
      power_rits_ipw <- power_rits_ipw + sapply(1:n_comp, function(t){
        rej <- stop_trial(intvs = rits_out[[i]]$contr_ipw[t, k, 2:3], 
                          K = K, zero = min_thresh)
        rej
      })
    }
