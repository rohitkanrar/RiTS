...
ts_mis_out <- readRDS("output/ts_mis_out.RData")
rits_mis_out <- readRDS("output/rits_mis_out.RData")
rand_mis_out <- readRDS("output/rand_mis_out.RData")
sim_dat <- readRDS("output/sim_dat.RData")

N <- length(ts_mis_out$trt)
...
# regret utility
df <- data.frame(patient = 1:N, ts = cumsum(ts_mis_out$regret),
                 rand = cumsum(rand_mis_out$regret), 
                 rits = cumsum(rits_mis_out$regret))
df_long <- tidyr::gather(df, key = "strategy", value = "value", -patient)

# Plot using ggplot2
plot1 <- ggplot(df_long, aes(x = patient, y = value, color = strategy)) +
  geom_line() +
  geom_vline(xintercept = ts_mis_out$tr_first, linetype = "dashed", color = "blue") +
  labs(title = "Utility Regret",
       x = "Patient", y = "Cumulative Regret") +
  theme(text = element_text(size = 8))

df <- data.frame(patient = 1:N, ts = cumsum(ts_mis_out$subopt_trt),
                 rand = cumsum(rand_mis_out$subopt_trt),
                 rits = cumsum(rits_mis_out$subopt_trt))
df_long <- tidyr::gather(df, key = "strategy", value = "value", -patient)
...
ggsave("plot/regret_mis_all.jpg", height = 3, width = 6, units = "in")


# regret benefit
df <- data.frame(patient = 1:N, ts = cumsum(ts_mis_out$regret_benf),
                 rand = cumsum(rand_mis_out$regret_benf), 
                 rits = cumsum(rits_mis_out$regret_benf))
df_long <- tidyr::gather(df, key = "strategy", value = "value", -patient)
...
df <- data.frame(patient = 1:N, ts = cumsum(ts_mis_out$subopt_trt_benf),
                 rand = cumsum(rand_mis_out$subopt_trt_benf),
                 rits = cumsum(rits_mis_out$subopt_trt_benf))
df_long <- tidyr::gather(df, key = "strategy", value = "value", -patient)
...
plot12 <- plot1 + plot2 + plot_layout(ncol = 2, guides = "collect")
ggsave("plot/regret_mis_benf.jpg", height = 3, width = 6, units = "in")

# regret safety
df <- data.frame(patient = 1:N, ts = cumsum(ts_mis_out$regret_safe),
                 rand = cumsum(rand_mis_out$regret_safe), 
                 rits = cumsum(rits_mis_out$regret_safe))
df_long <- tidyr::gather(df, key = "strategy", value = "value", -patient)
...
df <- data.frame(patient = 1:N, ts = cumsum(ts_mis_out$subopt_trt_safe),
                 rand = cumsum(rand_mis_out$subopt_trt_safe),
                 rits = cumsum(rits_mis_out$subopt_trt_safe))
df_long <- tidyr::gather(df, key = "strategy", value = "value", -patient)
...
plot12 <- plot1 + plot2 + plot_layout(ncol = 2, guides = "collect")
ggsave("plot/regret_mis_safe.jpg", height = 3, width = 6, units = "in")



# plotting propensity
K <- length(unique(rand_mis_out$trt))
df <- as.data.frame(ts_mis_out$log_dat$prpns_mat)
...
plot_prop_ts <- ggplot(df_long, aes(x = obs, y = Prpn, color = Arm)) +
  ...
  labs(x = "Patient", y = "Proprnsity", 
       title = "Thompson Sampling") +
  theme(text = element_text(size = 10))

df <- as.data.frame(rits_mis_out$log_dat$prpns_mat)
colnames(df) <- paste("Arm", 1:K)
df$obs <- 1:N  # Adding a column for observation number
...
ggsave("plot/prop_mis_all.jpg", height = 4, width = 9, units = "in")


ate_start <- ts_mis_out$tr_first
ate <- ts_mis_out$ate
# ATE
## TS
df_ts <- data.frame(
...
ggsave("plot/ate_mis_ts_rand.jpg", height = 3, width = 9, units = "in")
...
ggsave("plot/contr_mis_ts_rand.jpg", height = 3, width = 9, units = "in")